...
import { register, loading } from '../store/slices/AuthSlice'
...
const Register = () => {
  ...
  return (
    <div className=' mx-auto my-28 w-96  dark:text-white bg-slate-300 text-center rounded-xl p-5' >
      <form onSubmit={(e) => submit(e)} >

        <div className='  text-3xl'>LogIn</div>
        <div className='flex flex-col mx-auto w-[90%]'>
          <label htmlFor="email" className='self-start mb-1'>Name</label>
          <input type="text" name='name' required className=' self-center outline-none border-none px-5 py-2 rounded-md w-full ' />
        </div>
        <div className='flex flex-col mx-auto w-[90%]'>
        ...
        <div className='flex flex-col mx-auto w-[90%]'>
          <label htmlFor="Password" className='self-start mb-1'>Confirm Password</label>
          <input type="Password" style={err && { border: '1px solid red' }} name='Passwordc' required className=' self-center outline-none border-none px-5 py-2 rounded-md w-full ' />
        </div>
        <button className=' mx-6 bg-indigo-600 hover:bg-indigo-800 transition-all w-[85%] py-2 rounded-2xl my-5 text-white'>Sign up</button>
        <div >already have an account? <span className=' underline text-blue-500 cursor-pointer hover:text-blue-700' onClick={() => navigate('/signIn')}> sign in</span></div>
      ...
      <button onClick={(e) => submit(e, 'google')} className=' mx-auto mt-5 w-[75%] flex bg-blue-600 hover:bg-blue-700 text-white p-1 rounded-xl'>
        ...
        <span className=' text-lg ml-4'>continue with Google</span>
      </button>
