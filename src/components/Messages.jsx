    ...
    return (<div className=' w-full h-full flex flex-col justify-start pb-5'>
        ...
        {messages.map(msg =>
            <div key={msg.date} className={`flex ${msg.senderId == user.uid ? 'flex-row-reverse' : 'flex-row'} my-5 `}>
                ...
                <div className={`${msg.senderId == user.uid ? 'bg-green-700' : 'bg-slate-500'} max-w-[220px]  py-2 w-fit px-4 rounded-lg flex flex-col items-center justify-between `}>

                    {msg.img && <div className=' max-h-72 max-w-72 overflow-hidden  rounded-md -mx-2 cursor-pointer' onClick={() => setimg2show(msg.img)}>
                        <img src={msg.img} alt="" />
                    </div>}

                    <div className={` w-fit  rounded-lg max-w-[220px] break-words `}>{msg.text}</div>
